...
function M.wrap_tools(tools_module)
  if not tools_module or not tools_module.tools then
    return
  end
  ...
  tools_module.handle_invoke = function(client, params)
    ...
    -- Extract file paths for common tools
    if params.name == "openFile" and params.arguments and params.arguments.filePath then
      entry.file_path = params.arguments.filePath
    elseif params.name == "open_diff" and params.arguments then
      entry.file_path = params.arguments.filePath or params.arguments.targetFilePath
      entry.type = "diff"
